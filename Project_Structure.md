...
| `bootstrap_prompts/` | **Plan Archive**: Systematic prompts generated from user intent to start new sessions. *(Created at runtime by the Prompt Architect — `scripts/generate_bootstrap_prompt.py`; git-ignored, absent until first run.)* |
...
| Path | Purpose |
| :--- | :--- |
| `src/` | Application source code. *(Planned — not yet present on disk; this repo currently carries no application code.)* |
...
| 2026-07-05 | ADD | `specs/001-framework-governance-and-fleet-initiative/`, `specs/002-maintenance-hygiene-skills/`, `specs/003-prompt-architect-skill/` | **V0.0.4 — Retro-spec conversion (as-built Spec Kit sets).** Reverse-engineered code-grounded Spec Kit document sets (spec/plan/research/data-model/quickstart/tasks/contracts, tasks marked [X] as-built) for the three delivered capabilities: framework governance + fleet initiative (v0.0.1–v0.0.3), maintenance/hygiene skills (`scripts/`), and the Prompt Architect skill. All claims cited to HEAD `path:line`. Docs-only change — cost review: $0, no infra. |
| 2026-07-05 | FIX | `scripts/README.md`, `scripts/verify_structure.py`, `scripts/optimize_changelog.py`, `scripts/generate_bootstrap_prompt.py`, `scripts/update_getting_started.py`, `.gitignore`, `Project_Structure.md`, `PATTERNS.md`, `GEMINI.md` | **V0.0.5 — Script hygiene & docs-drift fixes** (backlog from specs 001/002/003). Fail-loud exits: LLM scripts now exit non-zero on API/model-listing failures (Automation-First CLI). Dynamic-model-only: removed hardcoded retired fallback model IDs from all three LLM scripts (Non-Hardcoded LLM Selection) and the `models/`-prefix inconsistency. Backported the EOF-safe interactive model picker to `update_getting_started.py`. `generate_bootstrap_prompt.py --dry-run` now works keylessly (short-circuit before the key check). Removed dead code in `verify_structure.py` (unused `import re`, unused `dry_run` plumbing). `scripts/README.md` now documents all 4 scripts with usage. `.gitignore` gains `__pycache__/` + `bootstrap_prompts/`. Hardened LLM-script stdout against legacy Windows codepages (`errors="replace"`) — previews containing `→` no longer crash with UnicodeEncodeError. Docs drift: repaired the broken changelog table row, annotated the `bootstrap_prompts/`/`src/` map rows, fixed the `Function_Mapping.md` path in PATTERNS.md, stripped residual citation artifacts from GEMINI.md (rules verbatim). Cost review: $0, no infra. Bruno/terraform: N/A (no API surface). |
| 2026-07-06 | ADD | `.claude/skills/prototype/` | **V0.0.6 — `prototype` skill (framework-generic).** Added a brand-neutral clickable-mockup generator: turns an existing `specs/NNN-slug/` into a self-contained, offline HTML/JS `prototype/` bundle for requirement validation (companion to `/speckit-specify` / `/retro-spec`; write-confined to `prototype/`; no network/CDN/secrets). De-branded equivalent authored for the framework so children forked from AVF inherit it. Skill-only change — cost review: $0, no infra. |
| 2026-07-06 | ADD | `.claude/skills/skill-distill/` | **V0.0.7 — `skill-distill` capability (framework-generic).** Added the per-repo skill-library generator (retiring-fellow method: discover → author 10–16 repo-specific runbooks → 3-reviewer pass) so each repo — the framework and every child forked from it — can distill ITS OWN handover library instead of inheriting another repo's. De-branded (no repo-specific references). NOT auto-run (a full run is EXPENSIVE — 30%+ of a weekly limit). Skill-only change — cost review: $0, no infra. |